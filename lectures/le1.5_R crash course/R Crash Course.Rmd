...
Please download the data we will be working with today and put it in a location you can find:

https://github.com/DataInvestigations/class_materials/

It is also on Canvas.

Easiest: Click the link above, Click the drop-down that says 'Code'. Under Local, click "download zip". Choose a location you can find again that is not your downloads folder. Extract the zip file, and double click on the file "class_materials.Rproj"

A simple function, read.csv(), that allows you to easily bring your .csv into the R environment. Simply run below line of code to read in your data: 

```{r}
batdat <- read.csv("datasets/bat_data.csv")
```
