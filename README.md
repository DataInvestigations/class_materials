README
================
Kate Langwig

------------------------------------------------------------------------

GitHub Documents
----------------
This repository contains the course documents. The files you'll need are given week by week. 

Course Schedule
----------------
[Topics by Week](https://docs.google.com/spreadsheets/d/1wrJyMB2GarLvCcgu6rTkHH0aZuFh_H-I84vAgMvf0Kw/edit?usp=sharing)

Week 1
-------------------
#### Tuesday: 

lecture: [week1.pptx](https://github.com/DataInvestigations/class_materials/tree/main/lectures/le1_intro_lecture/week1.pptx)

#### Thursday:
lecture: [week1_introR.pptx](https://github.com/DataInvestigations/class_materials/blob/main/lectures/le1.5_R%20crash%20course/week1_introR.pptx)
activity: [R-crash-course.pdf](https://github.com/DataInvestigations/class_materials/blob/main/lectures/le1.5_R%20crash%20course/R-Crash-Course.pdf)

##### Task list for next week:
- Submit screenshot of program installs

Week 2
-------------------
#### Tuesday: 

lecture: snowday

#### Thursday:
lecture: [AskingQuestions-ppt-act.pptx](https://github.com/DataInvestigations/class_materials/blob/main/lectures/le2_ask%20a%20question%20lecture/AskingQuestions-ppt-act.pptx)
activity: [Student Handout.docx](https://github.com/DataInvestigations/class_materials/blob/main/lectures/le2_ask%20a%20question%20lecture/Student%20Handout.docx)


Week 3
-------------------
#### Tuesday:
lecture: [le3_bio_background.pptx](https://github.com/DataInvestigations/class_materials/blob/main/lectures/le3_background/le3_bio_background.pptx)

#### Thursday:
activity: annotated bibliography

Week 4
--------------------
#### Tuesday:
lecture: le4_data_manipulation.html

#### Thursday:
lecture: ex4_data_manipulation.html
code: le4_data_manipulation.R
to do: data management assignment


Week 5
--------------------
#### Tuesday: 
lecture: le5_datapt2_got.html
code: git_token_help.R

### Thursday:
assignment: create group project repo and create a README

Week 6
--------------------
### Tuesday
lecture: le6_data_visualization.pptx
code: le6_data_visualization.R

### Thursday
lecture: ex6_data_visualization.pptx
assignment: individual ggplot2 assignment (use le6 code)

Week 7
--------------------
### Tuesday
lecture: RECORDED, le7_intro_to_stat_analysis.pptx
code: le7_intro_to_stat_analysis.R

### Thursday
lecture: None, no class
activity: take quiz on stat analysis


Week 8
--------------------
### Tuesday
lecture: RECORDED, le8_intro_linear_models.html
code: ex8_intro_to_linear_models.R

### Thursday
assignment: linear models, use code from Tuesday


Week 9
--------------------
### Tuesday
lecture: le9_Linear_model_parameters.html
code: ex9_linear_model_parameters.R

### Thursday
activity: group work on additive and interactive models

Week 10
--------------------
### Tuesday
lecture: le10_distributions.pptx

### Thursday
activity: work on preliminary report for group work,
see final_research_report.pptx


Week 11
--------------------
### Tuesday
lecture: le11_glms.pptx
code: le11_glms.R

### Thursday
assignment: glms
code: ex11_glms.R

